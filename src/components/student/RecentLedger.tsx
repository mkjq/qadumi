...
export default function RecentLedger({ transactions = [], submissions = [] }: RecentLedgerProps) {
  ...
  const getTransactionTypeLabel = (type: string) => {
    switch (type) {
      ...
      default:
        return { label: 'نقاط', icon: Star, color: 'text-slate-300 bg-gradient-to-b from-[#051124] via-[#091C3B] to-[#0A2246]' };
    }
  };

  return (
    <div className="rounded-3xl border border-white/10/80 bg-white/5/90 p-6 shadow-sm backdrop-blur-md">
      {/* Header Tabs */}
            ...
            {transactions.length > 0 ? (
              transactions.map((tx) => {
                ...
                return (
                  <div
                    key={tx.id}
                    className="flex items-center justify-between rounded-2xl border border-white/5 bg-gradient-to-b from-[#051124] via-[#091C3B] to-[#0A2246]/50 p-3.5 transition-colors hover:bg-gradient-to-b from-[#051124] via-[#091C3B] to-[#0A2246] hover:border-white/10"
                  >
          ...
          >
            {submissions.length > 0 ? (
              submissions.map((sub) => (
                <div
                  key={sub.id}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-2xl border border-white/5 bg-gradient-to-b from-[#051124] via-[#091C3B] to-[#0A2246]/50 p-4 transition-colors hover:bg-gradient-to-b from-[#051124] via-[#091C3B] to-[#0A2246] hover:border-white/10"
                >
                  <div className="flex items-center gap-3">
